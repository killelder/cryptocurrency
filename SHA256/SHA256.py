...
def sha_256(input_string):
    bit_main = message_pre_pro(input_string)
    M_t = []
    for chuck in range(int(len(bit_main)/512)):
        M_t.append(L_P(bit_main[chuck*512:(chuck+1)*512],32))
    #print(len(input_string),len(M_t),bit_main)
    #w_t=message_parsing(input_string)
    hash_0 = (dec_return_hex(initial_hash_values[0]))
    hash_1 = (dec_return_hex(initial_hash_values[1]))
    hash_2 = (dec_return_hex(initial_hash_values[2]))
    hash_3 = (dec_return_hex(initial_hash_values[3]))
    hash_4 = (dec_return_hex(initial_hash_values[4]))
    hash_5 = (dec_return_hex(initial_hash_values[5]))
    hash_6 = (dec_return_hex(initial_hash_values[6]))
    hash_7 = (dec_return_hex(initial_hash_values[7]))
    ...
    for chuck in range(int(len(bit_main)/512)):
        ...
        print(hex_return(hash_0),hex_return(hash_1),hex_return(hash_2),hex_return(hash_3),hex_return(hash_4),hex_return(hash_5),hex_return(hash_6),hex_return(hash_7))
        ...
        for i in range(0,16):
            w_t.append(M_t[chuck][i])
        for i in range(16,64):
            w_t.append(bin_32bit(mod_32_addition([int(sigma1(w_t[-2]),2),int(w_t[-7],2),int(sigma0(w_t[-15]),2),int(w_t[-16],2)])))
        ...
        for i in range(0,64):            
            ...
            a=bin_32bit(a)
            e=bin_32bit(e)
            #print(i,hex_return(int(a,2))," ",hex_return(int(b,2))," ",hex_return(int(c,2))," ",hex_return(int(d,2)),hex_return(int(e,2)))
        hash_0 = mod_32_addition([hash_0,int(a,2)])
        hash_1 = mod_32_addition([hash_1,int(b,2)])
        hash_2 = mod_32_addition([hash_2,int(c,2)])
        hash_3 = mod_32_addition([hash_3,int(d,2)])
        hash_4 = mod_32_addition([hash_4,int(e,2)])
        hash_5 = mod_32_addition([hash_5,int(f,2)])
        hash_6 = mod_32_addition([hash_6,int(g,2)])
        hash_7 = mod_32_addition([hash_7,int(h,2)])
...
print(sha_256("abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq"))
hash_object = hashlib.sha256(b'abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq')
